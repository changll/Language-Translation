...
%token NAME NUMBER AMPEREQUAL AMPERSAND AND AS ASSERT AT BACKQUOTE BAR BREAK CIRCUMFLEX
...
%type<id> NAME NUMBER
%type<val>  pick_PLUS_MINUS pick_multop augassign pick_unop comp_op

%type<node> trailer star_trailer arglist star_argument_COMMA pick_argument argument
%type<node> opt_test_2 break_stmt continue_stmt with_stmt while_stmt for_stmt classdef try_stmt decorated
...
%type<node> star_EQUAL yield_expr opt_yield_test comparison pick_yield_expr_testlist_comp pick_NEWLINE_stmt
%type<node> small_stmt flow_stmt return_stmt print_stmt del_stmt pass_stmt import_stmt assert_stmt
%type<node> global_stmt exec_stmt expr_stmt if_stmt suite funcdef stmt simple_stmt compound_stmt
%type<node> plus_stmt opt_arglist parameters varargslist opt_EQUAL_test star_fpdef_COMMA fpdef
%start start
...
pick_NEWLINE_stmt // Used in: star_NEWLINE_stmt
	...
	| stmt
    {
      if($1){
        ...
        std::cout<<"can't eval the whole program"<<std::endl;
      }
    }
	;
...
varargslist // Used in: parameters, old_lambdef, lambdef
    ...
    {
      Parameter* p = new Parameter(static_cast<IdentNode*>($2)->getIdent(), $3);
      pool.add(p);
      static_cast<ParaNode*>($1)->insert(p);
    }
	;
opt_EQUAL_test // Used in: varargslist, star_fpdef_COMMA
	: EQUAL test
    {$$ = new AsgBinaryNode($$,$2);pool.add($$);}
	| %empty
    {$$ = nullptr;}
	;
...
plus_stmt // Used in: suite, plus_stmt
	: plus_stmt stmt
    {
      static_cast<SuiteNode*>($1)->insert($2);
    }
	| stmt
    {
      $$ = new SuiteNode();
      static_cast<SuiteNode*>($$)->insert($1);
      pool.add($$);
    }
	;
...
power // Used in: factor
    ...
    {
      if($2){
        const std::string n = reinterpret_cast<IdentNode*>($1)->getIdent();
        $$ = new CallNode(n, ((ArgNode*)$2));
        pool.add($$);
      }
      else $$ = $1;
    }
...
arglist // Used in: opt_arglist
    ...
    {static_cast<ArgNode*>($1)->insert($2);
    }
	;
